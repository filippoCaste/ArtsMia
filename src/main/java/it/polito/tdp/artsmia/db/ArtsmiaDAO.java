...
public class ArtsmiaDAO {
	...
	public void listObjects(Map<Integer, ArtObject> idMap) {
		...
		try {
			...
			while (res.next()) {
				...
				if(!idMap.containsKey(res.getInt("object_id"))) { // se non contiene già questo oggetto, lo inserisco --> RISPARMIO DI NEW
	...
	//	--escludo tutto le coppie inverse così -- 
	...
	public List<Adiacenza> getAdiacenze(Map<Integer, ArtObject> idMap) {
		...
		try {
			...
			while(rs.next()) {
				adiacenze.add(new Adiacenza(idMap.get(rs.getInt("e1")), idMap.get(rs.getInt("e2")), rs.getInt("peso")));
			}
